...
bool ax::NodeEditor::Link(LinkId id, PinId startPinId, PinId endPinId, const ImVec4& color, float thickness, bool sameNode)
{
...
bool ax::NodeEditor::LinkDuplicates(const std::vector<std::pair<uint64_t, ImVec4>>& ids, PinId startPinId, PinId endPinId, float thickness, bool sameNode)
{
    ...
    const float starty = startPin->m_Node->GetBounds().GetTR().y;
...
void ax::NodeEditor::Flow(LinkId linkId, FlowDirection direction, const ImVec4& color)
{
